<template>
  <div id="chapter">
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item :to="{ path: '/index/mains/chapter' }">添加章节信息</el-breadcrumb-item>
    </el-breadcrumb>
    <div class="body">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async save() {
        if(this.input === null || this.content === null){
          this.$notify({
            message:"数据为空,请输入后保存",
            type:"error"
          });
        ...
        console.log(this.bookId + "------" + this.input + "------" + this.content);
        await AddChapter({
          bId: this.bookId,
          title: this.input,
          context: this.content,
          frontSequenceNum: null
        }).then((res) => {
          console.log(res);
          this.$notify({
            message: "保存成功!!!",
      ...
      /*async GetChapter(id) {//初始化获取章节
        console.log(id);
        await GetChapter({cid: id}).then((res) => {
          console.log(res);
    ...
    created() {
      this.bookId = JSON.parse(decodeURIComponent(this.$route.query.id));
      // console.log(id);
      // this.bookId = id;
      ...
      // const chapterId = JSON.parse(decodeURIComponent(this.$route.params.id));
